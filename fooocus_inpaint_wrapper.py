...
class FooocusInpaintWrapper:
	...
	def INPUT_TYPES(s):
		...
		return {
			...
			"optional": {
				...
				"outpainting": (["", "Left", "Right", "Top", "Bottom", "Top + Bottom", "Left + Right"],),
	...
	def start_periodic_check(self, percentage_file):
		pbar = comfy.utils.ProgressBar(100)
		# Simuliamo un controllo periodico (ad esempio, ogni 1 secondo)
		while True:
			if os.path.exists(percentage_file):
				with open(percentage_file, 'r') as f:
					percentage = f.read().strip()
					pbar.update_absolute(int(percentage))
			else:
				print(f"File {percentage_file} non trovato.")
			
			time.sleep(5)

	def start_inpaint(self, image, mask, performance, checkpoint, prompt, guidance_scale, image_sharpness, seed, negative_prompt, method, inpaint_additional_prompt, outpainting, lora1, lora1_weight, lora2, lora2_weight, lora3, lora3_weight, lora4, lora4_weight, lora5, lora5_weight):
